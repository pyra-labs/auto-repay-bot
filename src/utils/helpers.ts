...
export async function fetchExactOutParams(
	...
	const collateralBalanceValue =
		...
		collateralPrice;
	const loanEquivalentDecimal =
		(collateralBalanceValue / loanPrice) * (1 - SWAP_SLIPPAGE_BPS / 10_000);
	const loanEquivalentBaseUnits = decimalToBaseUnit(
		loanEquivalentDecimal,
		marketIndexLoan,
	);
	...
	const targetRepayAmountLoanBaseUnits = decimalToBaseUnit(
		loanRepayValue / loanPrice,
		marketIndexLoan,
	);

	const repayAmountLoan = Math.min(
		targetRepayAmountLoanBaseUnits,
...
export async function fetchExactInParams(
	...
	const targetRepayAmountCollateralBaseUnits = decimalToBaseUnit(
		targetRepayAmountCollateralDecimal,
		marketIndexCollateral,
	);
	const repayAmountCollateral = Math.min(
		targetRepayAmountCollateralBaseUnits,
		collateralBalance,
	);
...
export async function getRepayMarketIndices(user: QuartzUser) {
	...
	const balances = balancesArray.reduce(
		(acc, { index, balance }) => Object.assign(acc, { [index]: balance }),
		{} as Record<MarketIndex, BN>,
	);

	let lowestBalance: { index: MarketIndex; balance: BN } = {
		index: MarketIndex[0],
		balance: balances[MarketIndex[0]],
	};
...
async function getPricesPyth(): Promise<Record<MarketIndex, number>> {
	const pythPriceFeedIdParams = MarketIndex.map(
...
async function getPricesCoinGecko(): Promise<Record<MarketIndex, number>> {
	const coinGeckoIdParams = MarketIndex.map(
		(index) => TOKENS[index].coingeckoPriceId,
	).join(",");
