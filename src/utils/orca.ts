...
import { buildEndpointURL, DummyWallet, fetchAndParse, TOKENS, BN } from "@quartz-labs/sdk";
...
export async function getOrcaSwapIx(
    ...
    const fromToken = Object.values(TOKENS).find(token => token.mint === fromMint);
    ...
    const whirlpool = await whirlpoolClient.getPool(new PublicKey(whirlpoolPubkey));
    ...
    const {
        instructions,
    } = txBuilder.compressIx(true);

    const swapIx = instructions.find(
        ix => ix.programId.equals(ORCA_WHIRLPOOL_PROGRAM_ID)
    );
    if (!swapIx) {
